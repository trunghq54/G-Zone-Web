    ...
    <div className="mx-auto w-full max-w-5xl px-4 py-10">
      <div className="mb-6 flex items-center justify-between border-b border-surface-border pb-4">
        <div>
          ...
          <p className="mt-1 text-text-muted text-sm">Track your COD order progress and payment status.</p>
        </div>
                ...
                >
                  ...
                  <div className="flex items-center gap-3 sm:gap-6 flex-wrap sm:flex-nowrap shrink-0">
                    <div className="text-right hidden sm:block">
                      ...
                      <p className="text-white text-sm font-bold">{order.orderDetails?.length ?? 0}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-xs text-text-muted">Total</p>
                      <p className="text-primary font-black text-base">${order.totalAmount.toFixed(2)}</p>
                    </div>
                    <span className={`rounded-lg border px-3 py-1 text-xs font-bold uppercase whitespace-nowrap ${statusClass(order.status)}`}>
                      {order.status}
                    </span>
                    <span className={`material-symbols-outlined text-text-muted text-base transition-transform ${isExpanded ? 'rotate-180' : ''}`}>
                      expand_more
                    </span>
                  </div>
                </button>
